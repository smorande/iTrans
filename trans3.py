...
class SmartScriptOCR:
    ...
    def initialize_ocr(self, lang: str) -> None:
        """Initialize OCR reader for specified language"""
        lang_code = self.supported_languages[lang][0]
        self.easy_reader = easyocr.Reader([lang_code], gpu=False)
        
    def calculate_confidence(self, ocr_results: list) -> float:
    ...
    def process_traditional(self, image: np.ndarray) -> Tuple[str, float]:
        """Process image using traditional OCR"""
        try:
            if self.easy_reader is None:
                raise ValueError("OCR reader not initialized")
            easy_result = self.easy_reader.readtext(image)
            text = ' '.join([res[1] for res in easy_result]).strip()
            confidence = self.calculate_confidence(easy_result)
